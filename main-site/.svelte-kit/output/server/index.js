...
async function handle_fatal_error(event, options, error2) {
  ...
  if (
    has_data_suffix(new URL(event.request.url).pathname) ||
    ...
    return action_json(
      {
        ...
        error: await handle_error_and_jsonify(
          event,
          options,
          check_incorrect_fail_use(error2)
        )
      },
      {
      ...
      .map(({ server_data }) => {
        if (server_data?.type === "data") {
          ...
          if (server_data.uses.dependencies.size > 0) {
            uses.push(
              `dependencies:${s(Array.from(server_data.uses.dependencies))}`
            );
          }
  ...
  try {
    ...
    try {
      ...
      return json_response(
        JSON.stringify(clarify_devalue_error(event, error2)),
        500
      );
    }
  } catch (e) {
    ...
    if (error2 instanceof Redirect) {
      ...
      return json_response(
        JSON.stringify(await handle_error_and_jsonify(event, options, error2))
      );
    }
  }
}
...
function get_cookies(request, url, dev, trailing_slash) {
  ...
  if (dev) {
    const initial_decoded_cookies = parse(header, {
    ...
    for (const name of Object.keys(cookie_paths)) {
      cookie_paths[name] = new Set(
        [...cookie_paths[name]].filter(
          (path) =>
            !path_matches(normalized_url, path) ||
            name in initial_decoded_cookies
        )
      );
    }
  ...
  const cookies = {
    get(name, opts) {
      ...
      if (paths.size > 0) {
        console.warn(
          `Cookie with name '${name}' was not found at path '${
            url.pathname
          }', but a cookie with that name exists at these paths: '${[
            ...paths
...
async function respond(request, options, state) {
  ...
  if (options.csrf.check_origin) {
    ...
    if (forbidden) {
      ...
      return new Response(csrf_error.body.message, {
  ...
  constructor(manifest) {
    this.options = {
      ...
      handle_error: (error2, event) => {
        return (
          this.options.hooks.handleError({ error: error2, event }) ?? {
