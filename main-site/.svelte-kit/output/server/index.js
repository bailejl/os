...
${
  components[1]
    ...
    : `${validate_component(
        components[0] || missing_component,
        "svelte:component"
      ).$$render($$result, { data: data_0, form }, {}, {})}`
}
...
function is_content_type(request, ...types) {
  var _a;
  const type =
    ((_a = request.headers.get("content-type")) == null
      ? void 0
      : _a.split(";", 1)[0].trim()) ?? "";
...
function data_response(data, event) {
  ...
  try {
    ...
    const message = match
      ? `Data returned from \`load\` while rendering ${event.routeId} is not serializable: ${error2.message} (data.${match[2]})`
      : error2.message;
...
function get_option(nodes, option) {
  return nodes.reduce((value, node) => {
    ...
    return (
      ((_a = node == null ? void 0 : node.shared) == null
        ? void 0
        : _a[option]) ??
      ((_b = node == null ? void 0 : node.server) == null
        ? void 0
        : _b[option]) ??
      value
    );
    ...
    return action_json(
      {
        ...
        error: handle_error_and_jsonify(
          event,
          options,
          check_incorrect_invalid_use(error2)
        )
      },
      {
...
async function unwrap_promises(object) {
  ...
  for (const key2 in object) {
    if (
      typeof ((_a = object[key2]) == null ? void 0 : _a.then) === "function"
    ) {
...
async function load_server_data({ event, state, node, parent }) {
  ...
  return {
    ...
    uses: {
      dependencies:
        uses.dependencies.size > 0 ? Array.from(uses.dependencies) : void 0,
...
async function load_data({
  ...
  if (
    !((_a = node == null ? void 0 : node.shared) == null ? void 0 : _a.load)
  ) {
      ...
      if (same_origin) {
        ...
        const mode =
          input instanceof Request
            ? input.mode
            : (init2 == null ? void 0 : init2.mode) ?? "cors";
        if (mode !== "no-cors") {
...
class CspReportOnlyProvider extends BaseProvider {
  constructor(use_hashes, directives, nonce, dev) {
    ...
    if (Object.values(directives).filter((v) => !!v).length > 0) {
      const has_report_to =
        ((_a = directives["report-to"]) == null ? void 0 : _a.length) ?? 0 > 0;
...
class Csp {
  ...
  constructor({ mode, directives, reportOnly }, { prerender, dev }) {
    ...
    this.csp_provider = new CspProvider(
      use_hashes,
      directives,
      this.nonce,
      dev
...
async function render_response({
  ...
  const prefixed = (path) =>
    path.startsWith("/") ? path : `${assets2}/${path}`;
  const serialized = { data: "", form: "null" };
  try {
    serialized.data = devalue.uneval(
    ...
    if (nonPojoError) {
      const constructorName =
        (_e =
          (_d =
            (_c =
              (_b = branch.find(({ server_data }) => {
                var _a2, _b2;
                return (_b2 =
                  (_a2 = server_data == null ? void 0 : server_data.data) ==
                  null
                    ? void 0
                    : _a2.constructor) == null
                  ? void 0
                  : _b2.name;
              })) == null
                ? void 0
      ...
      throw new Error(
        `Data returned from \`load\` (while rendering ${
          ...
          constructorName
            ? ` rather than an instance of ${constructorName}`
            : ""
        }`
      );
    }
...
async function render_page(event, route, page, options, state, resolve_opts) {
  ...
  try {
    const nodes = await Promise.all([
      ...page.layouts.map((n) =>
        n == void 0 ? n : options.manifest._.nodes[n]()
      ),
    ...
    if (is_action_request(event, leaf_node)) {
      ...
      if (
        (action_result == null ? void 0 : action_result.type) === "redirect"
      ) {
    ...
    const should_prerender_data = nodes.some((node) =>
      node == null ? void 0 : node.server
    );
    ...
    if (state.prerendering && should_prerender_data) {
      const body = devalue.stringify({
        ...
        nodes: branch.map((branch_node) =>
          branch_node == null ? void 0 : branch_node.server_data
        )
      });
      state.prerendering.dependencies.set(data_pathname, {
        response: new Response(body),
...
async function render_data(event, route, options, state) {
  ...
  try {
    ...
    const invalidated =
      ((_a = event.request.headers.get("x-sveltekit-invalidated")) == null
        ? void 0
        : _a.split(",").map(Boolean)) ?? node_ids.map(() => true);
    let aborted = false;
    ...
    if (error2 instanceof Redirect) {
      ...
      return data_response(
        handle_error_and_jsonify(event, options, error2),
        event
      );
    }
  }
...
function get_cookies(request, url, options) {
  ...
  const cookies = {
    get(name, opts) {
      ...
      const decode =
        (opts == null ? void 0 : opts.decode) || decodeURIComponent;
...
async function respond(request, options, state) {
  ...
  if (
    options.paths.base &&
    !((_a = state.prerendering) == null ? void 0 : _a.fallback)
  ) {
  ...
  if ((route == null ? void 0 : route.page) && !is_data_request) {
    ...
    if (
      ...
      !((_c = state.prerendering) == null ? void 0 : _c.fallback)
    ) {
      return new Response(void 0, {
        status: 301,
        headers: {
