...
import * as browserLogs from "./tools/browser-logs";
import dataManager from "./data/data-manager";
import { removeSync } from "fs-extra";

// Some of the code came from https://github.com/contino/cucumber-declarative-gherkin
...
const enforceConsoleLogErrors =
  process.env.ENFORCE_CONSOLE_LOG_ERRORS || "true";
...
function logConsoleOutput(suppressErrors: boolean) {
  if (browserLogsInstance) {
    ...
    if (
      cucumberJson.attach &&
      bLogs &&
      consoleLoggingLevels &&
      consoleLoggingLevels.length > 0
    ) {
      bLogs.forEach((log: any) => {
        if (
          log &&
          log.level &&
          consoleLoggingLevels.includes(log.level.toString())
        ) {
          cucumberJson.attach(
            `${new Date(log.timestamp)} [${log.level}]: ${log.message}`
          );
        }
      });
      if (
        !suppressErrors &&
        enforceConsoleLogErrors &&
        enforceConsoleLogErrors.toLowerCase() === "true"
      ) {
        // browserLogsInstance.verify();
      }
    }
  } else {
    if (cucumberJson.attach)
      cucumberJson.attach(`Appears scenario was skipped, so no browser logs`);
  }
}
...
export const config: Options.Testrunner = {
  ...
  //
  specs: ["web/e2e/features/**/*.feature"],
  // Patterns to exclude.
  exclude: [
  ...
  capabilities: [
    {
      ...
      "goog:loggingPrefs": { driver: "WARNING", browser: "ALL" },
    },
  ],
  //
  // ===================
  ...
  cucumberOpts: {
    ...
    require: ["web/e2e/features/step-definitions/steps.ts"],
